...
//         const taxDecalarations = this.selectedData.filter((data) => { return data.incomeTaxComponent === taxComponent._id });
    ...
    this.taxService.getAllTaxComponents(pagination).subscribe((res: any) => {
      ...
      const formArray = this.fb.array(
        this.taxComponents.map(component => {
          const taxDeclaration = this.selectedData?.incomeTaxDeclarationComponent?.find(
            data => data.incomeTaxComponent === component._id
          );
          return this.createFormGroup(taxDeclaration || component);
        })
      );
      this.incomeTaxDecComponentForm.setControl('incomeTaxComponent', formArray);
