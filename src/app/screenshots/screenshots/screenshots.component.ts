...
export class ScreenshotsComponent implements OnInit {
  ...
  var result  = this.timeLogService.getLogsWithImages(currentUser.email, this.formatDate(this.selectedDate));
  result.subscribe({
    next: data => {
      this.screenshotRows = [];
...
addDays(date: Date, days: number): Date {
  ...
  formatDate(dateVal) {
    ...
    return sMonth + "-" + sDay + "-" + sYear + " 12:00 AM";
}
            ...
            for( let i=0; i<timeLogs.length;i++)
            {
               let fileName = this.GetFileNameWithoutExtension( timeLogs[i].filePath);
                ...
                if (hh==r && mm <= (c * 10 + 9) && mm>=(c*10)){
                    result = new  screenShotCell(`${hh}:${(mm - (mm % 10))}`,timeLogs[i].fileString,timeLogs[i].clicks,timeLogs[i].keysPressed,true);
                }
              };
          }
            return result;
